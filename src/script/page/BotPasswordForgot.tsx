...
const BotPasswordForgot = () => {
  ...
  const initiatePasswordReset = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await accountAction.initiateBotPasswordReset(emailInputNode.value);
      setSuccess(true);
    } catch (error) {
      if (error instanceof ValidationError) {
        ...
        switch (error.code) {
          ...
          default: {
            setError(t('errorUnknown'));
            console.warn('Failed to initiate bot password reset', error);
          }
        }
      }
    }
...
export default BotPasswordForgot;
